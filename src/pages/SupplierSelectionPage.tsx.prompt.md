...
## Description
The SupplierSelectionPage allows the user to select a supplier for a specific item that needs to be reordered.

## Screen layout
The SupplierSelectionPage will have a header, a side navigation bar, and a main content area. The main content area will have a form for selecting a supplier and a button to confirm the selection.

## Screen behavior
When the SupplierSelectionPage is loaded, it will display a list of available suppliers for the selected item. The user can select a supplier from the list and click the "Confirm" button to proceed to the next page. If the user clicks the "Cancel" button, they will be redirected back to the ReorderTriggerPage.

## Input Form
The SupplierSelectionPage will have a form with the following fields:
- Supplier selection dropdown: A dropdown menu that displays a list of available suppliers for the selected item.
- Confirm button: A button that confirms the selected supplier and proceeds to the next page.
- Cancel button: A button that cancels the selection and redirects the user back to the ReorderTriggerPage.

## Error messages
If there are no available suppliers for the selected item, the SupplierSelectionPage will display an error message: "No suppliers available for this item."
...
# prompt
Please carefully review the design information up to this point and create the tsx for the SupplierSelectionPage, keeping in mind the division of roles according to the screen list.
Please be sure to inspect the following points before submitting your work.
- Please use Chakra UI to create a polished design.
- Calibrate the screen with only the given components.
- screen should be for Japanese.
- Note the component names (especially the suffixes).
Please respond only to SupplierSelectionPage.tsx.
