...
## Description
The Track Shipment Page allows users to track the status of a shipment by entering the tracking number. The page displays the current status of the shipment and any relevant updates.

## Screen layout
The Track Shipment Page consists of a single input form and a section to display the shipment status.

The input form includes a text input field for the user to enter the tracking number and a submit button to initiate the search.

The shipment status section displays the current status of the shipment, including the carrier name, tracking number, and any relevant updates.

## Screen behavior
When the user enters a valid tracking number and clicks the submit button, the system will call the `trackShipment(trackingNumber: string): Promise<ShipmentStatus>` method from the `ShippingService` class to retrieve the shipment status.

If the tracking number is invalid or the shipment cannot be found, an error message will be displayed.
...
- ShippingService: trackShipment(trackingNumber: string): Promise<ShipmentStatus>
## Elements to be used
### React Elements
None
### React Dialogs
...
# prompt
Please carefully review the design information up to this point and create the tsx for the TrackShipmentPage, keeping in mind the division of roles according to the screen list.
Please be sure to inspect the following points before submitting your work.
- Please use Chakra UI to create a polished design.
- Calibrate the screen with only the given components.
- screen should be for Japanese.
- Note the component names (especially the suffixes).
Please respond only to TrackShipmentPage.tsx.
