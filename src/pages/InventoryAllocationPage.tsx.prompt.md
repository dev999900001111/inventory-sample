...
## Description
The Inventory Allocation Page allows users to view the current inventory allocation status for each item and expected shipments. Users can also adjust the allocation quantity for each item and view the updated expected shipments.

## Screen layout
The Inventory Allocation Page will consist of a table displaying the following columns:
- Item Name
- Item Description
- Allocated Quantity
- Available Quantity
...
Each row will represent an item in the inventory. The table will also have a button to trigger the adjustment of inventory allocation.

## Screen behavior
- On page load, the table will display the current inventory allocation status for each item and expected shipments.
- Users can adjust the allocation quantity for each item by clicking on the "Adjust Allocation" button.
- Clicking on the "Adjust Allocation" button will open a dialog box where users can enter the new allocation quantity for each item.
- After adjusting the allocation quantity, the table will update to display the updated allocation status and expected shipments.

## Input Form
The "Adjust Allocation" button will open a dialog box with the following input form:
- Item Name (read-only)
- Item Description (read-only)
- Allocated Quantity (input field)
- Available Quantity (read-only)
- Expected Shipments (read-only)

## Error messages
- If the user enters an invalid allocation quantity (e.g. negative number or non-numeric value), an error message will be displayed prompting the user to enter a valid quantity.
...
- InventoryService: getInventoryAllocation(), executeShipment()
## Elements to be used
### React Elements
None
### React Dialogs
...
# prompt
Please carefully review the design information up to this point and create the tsx for the InventoryAllocationPage, keeping in mind the division of roles according to the screen list.
Please be sure to inspect the following points before submitting your work.
- Please use Chakra UI to create a polished design.
- Calibrate the screen with only the given components.
- screen should be for Japanese.
- Note the component names (especially the suffixes).
Please respond only to InventoryAllocationPage.tsx.
